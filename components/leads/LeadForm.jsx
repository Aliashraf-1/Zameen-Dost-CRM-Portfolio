...
export default function LeadForm({
  ...
  const [form, setForm] = useState({
    ...
    createdBy: initialData?.createdBy || user?._id || user?.id || employee?._id || employee?.id || null,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit && initialData) {
        ...
        leadData._id = initialData._id || initialData.id;
        leadData.id = initialData._id || initialData.id;
      } else {
            ...
            <div className="grid grid-cols-2 gap-4">
              {form.assignedToName && (
                <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-300">{form.assignedToName}</p>
                </div>
              )}
              {form.createdByName && (
                <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3">
                  <p className="text-xs text-slate-500">Added By</p>
                  <p className="mt-1 text-sm font-medium text-slate-300">{form.createdByName}</p>
                </div>
              )}
            </div>
